...
class PaymentService:
    ...
    @staticmethod
    def create_payment_intent(amount: float, payment_method: str):
        """
        Like M-Pesa STK Push - initiates the payment process
        
        amount: in CHF (e.g., 150.50)
        payment_method: "card" or "twint"
        """
        
        # Stripe uses cents, like M-Pesa uses cents
        # CHF 150.50 → 15050 (multiply by 100)
        amount_in_cents = int(amount * 100)

        intent = stripe.PaymentIntent.create(
            amount=amount_in_cents,
            currency="chf",
            payment_method_types=[payment_method],  # "card" or "twint"
            metadata={
                "payment_method": payment_method
            }
        )

        # client_secret is like M-Pesa CheckoutRequestID
        # Frontend uses this to complete the payment
        return {
